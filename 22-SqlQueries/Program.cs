...
//Eğer ki, sorgunuzu LINQ ile ifade edemiyorsanız yahut lINQ'in ürettiği sorguya nazaran daha optimize bir sorguyu manuel
//geliştirmek ve EF Core üzerinden execute etmek istiyorsanız EF Core'un bu davrnaışı desteklediğini bilmelisiniz.

//Manuel bir şekilde/tarafımızca oluşturulmuş olan sorguları EF Core tarafından execute edebilmek için o sorgunun sonucunu
//karşılayacak bir entity model'ın tasarlanmış ve bunun DbSet olarak context nesnesine tanımlanmış olması gerekiyor.

#region FromSqlInterpolated
//EF Core 7.0 sürümünden önce ham sorguları execute edebildiğimiz fonksiyondur.
//var persons = await context.Persons.FromSqlInterpolated($"SELECT * FROM Persons") //IQuerable halde
//    .ToListAsync();

//.Persons dedik cunku sorguları EF Core tarafından execute edebilmek için o sorgunun sonucunu karşılayacak bir entity model'ın tasarlanmış 
//olması gerekir dedik. Bu sebeple tabloyu belirtiyoruz. Donen veri de bir person olması gerekir.
//Kısacası person sorgulayacaksam person olan bir db set in olması gerekir.

//Ayrıca FromSqlInterpolated fonksiyonu FormattableString tipinde bir deger bekler yani string interpolition kullanılan bir parametre bekler.
//FormattableString formattableString = ""; hata verir.
//FormattableString formattableString = $""; hata vermez
#endregion
...
//burada bir stored procedure tetikliyoruz.
#endregion
...
//Lakin dinamik olarak sorguları oluşturuyorsanız eğer burada güvenlik geliştirici sorumludur. Yani gelen sorguda/veri
//yorumlar, noktalı virgüller yahut SQL'e özel karakterlerin algılanması ve bunların temizlenmesi geliştirici tarafından gerekmektedir.
#endregion
...
//    .ToListAsync();
//Entity olmadıgı icin dbsete de gidemiyorum. Bu sebeple database e gidiyorum.
 
//Ayrıca sql yazıp sorgulama yaptıgımız yapılarda da devamında linq sorgularından yararlanabiliriz.
...
//var data = await context.Database.SqlQuery<int>($"SELECT PersonId Value FROM Persons") 
        //burada sorgu arka planda bir subquery olarak olusturulur. Bu sebeple butun sonucu value uzerine aldık ki hata almayalım.
//    .Where(x => x > 5)
//    .ToListAsync();

//SqlQuery'de LINQ operatörleriyle sorguya ekstradan katkıda bulunmak istiyorsanız eğer bu sorgu neticesinde gelecek olan kolonun
//adını Value olarak bildirmeniz gerekmektedir. Çünkü, SqlQuery metodu sorguyu bir subquery olarak generate etmektedir. Haliyle bu
//durumdan dolayı LINQ ile verilen şart ifadeleri statik olarka Value kolonuna göre tasarlanmıştır. O yüzden bu şekilde bir çalışma
//zorunlu gerekmektedir.
